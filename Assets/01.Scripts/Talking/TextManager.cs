...
public class TextManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (!IsClearForText()) return;
            if (!InspectionManager.instance.isOnCapture)
            {
                if(MapManager.instance.changeEffect.seq.IsPlaying())
                {
                    return;
                }
                StopAllCoroutines();
                ...
                {
                    Debug.Log(commentIdx + "번의 이벤트는 " + GetCurrentEvent().evtType);
                    if (GetCurrentEvent().evtType == TalkEventType.MapMove)
                    {
                        //if (!MapManager.instance.maps.ContainsKey(GetCurrentEvent().target1Key))
                        //{
    ...
    public void OpenSCG(CommentData data)
    {
        ...
        if (cgDictionary.ContainsKey(data.name) && data.name != "야루오")
        {
    ...
    {
        if (commentIdx > 0 && currentComment.texts[commentIdx-1].evt.evtType == TalkEventType.MapMove)
        {
            //if (!MapManager.instance.maps.ContainsKey(GetCurrentEvent().target1Key))
            //{
            //    Debug.Log(GetCurrentEvent().target1Key);
            //    MapManager.instance.OnMapUnlocked(MapManager.instance.GetMap(GetCurrentEvent().target1Key));
            //}
            MapManager.instance.ChangeMap(currentComment.texts[commentIdx - 1].evt.target1Key);
        }

        while (MapManager.instance.changeEffect.seq.IsPlaying())
        {
            yield return null;
        }
    ...
    public bool IsClearForText()
    {

        return !MapManager.instance.mapPanel.activeSelf && !QuestionManager.instance.questionPanel.activeSelf  && !InventoryManager.instance.inventoryPanel.activeSelf;
    }
    public bool IsClearForMap()
    {

        return !QuestionManager.instance.questionPanel.activeSelf  && !InventoryManager.instance.inventoryPanel.activeSelf;
    }
    public bool IsClearForQuestion()
    {

        return !MapManager.instance.mapPanel.activeSelf && !InventoryManager.instance.inventoryPanel.activeSelf;
    }
    public bool IsClearForInventory()
    {

        return !QuestionManager.instance.questionPanel.activeSelf && !MapManager.instance.mapPanel.activeSelf;
    }
